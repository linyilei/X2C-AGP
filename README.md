...
X2C-AGP 是一个 Android 编译期布局生成库。`x2c-compiler` 会在 APT 阶段扫描 `@Xml(layouts = "...")` 声明的入口布局，递归展开 `include` 依赖并生成对应的 Java View 构造代码；Gradle 插件负责传递 variant/resource 参数、聚合 library module index，并做 ASM 注入。运行时优先使用生成代码创建 View，未命中或暂不支持时自动回退到原生 `LayoutInflater`。
...
- APT 按 Android variant 读取 `res/layout*`，但只从 `@Xml(layouts = "...")` 入口递归展开生成目标。
...
- 支持 Android app 和 Android library 模块，library 会生成 group 和模块索引，app 会从依赖 classpath 聚合模块索引，并通过 ASM 注入 root index / module index 的直接注册代码；runtime root 查找走 AGP ASM instrumentation 注入，不再依赖旧 Transform API，运行时不再通过 `Class.forName` 查找 root 或 group。
...
本地调试时可以先发布到 `mavenLocal()`，当前示例版本为 `v0.1.1`；对外发布时仍可通过 JitPack 使用对应 tag。
...
buildscript {
    ...
    dependencies {
        classpath 'com.github.linyilei.X2C-AGP:x2c-gradle-plugin:v0.1.1'
    }
}
...
被 `include` 引用的布局不需要重复声明，插件会从入口布局开始递归收集依赖。
...
```text
D/X2C: Loaded generated root index: your.application.id.x2c.X2CRootIndex
...
- Android library 模块
- app 生成 root 索引，聚合依赖 classpath 中的 library 模块索引，并通过 ASM 把 root index 首次加载和模块索引注册都改成直接调用；runtime root 查找使用 AGP ASM instrumentation，不再注册旧 Transform
