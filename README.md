...
- 当 `include` 的目标布局是 `<merge>` 根节点时，X2C 与原生 `LayoutInflater` 行为完全对齐：会直接忽略 `include` 标签自身携带的 `LayoutParams`、`android:id` 或 `android:visibility`，将 merge 子节点直接挂载到父容器中，以确保 UI 层级严格一致。
