...
- 支持 Android app 和 Android library 模块，library 会生成 group 和模块索引，app 会从依赖 classpath 聚合模块索引，并通过 ASM 注入直接注册代码，library group 按首次使用懒加载。
...
- Android library 模块
- app 生成 root 索引，聚合依赖 classpath 中的 library 模块索引，并在 javac 后通过 ASM 把模块索引注册改成直接调用
