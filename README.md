...
X2C-AGP 是一个 Android 编译期布局生成库。它会在 Gradle 构建阶段扫描 `@Xml(layouts = "...")` 声明的入口布局，递归展开 `include` 依赖并生成对应的 Java View 构造代码；运行时优先使用生成代码创建 View，未命中或暂不支持时自动回退到原生 `LayoutInflater`。
...
- 按 Android variant 扫描 `res/layout*`，但只从 `@Xml(layouts = "...")` 入口递归展开生成目标。
