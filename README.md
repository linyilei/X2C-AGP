...
- 支持 Android app 和 Android library 模块，library 会生成 group 和模块索引，app 会从依赖 classpath 聚合模块索引，并通过 ASM 注入 root index / module index 的直接注册代码；运行时不再通过 `Class.forName` 查找 root 或 group。
...
- Android library 模块
- app 生成 root 索引，聚合依赖 classpath 中的 library 模块索引，并通过 ASM 把 root index 首次加载和模块索引注册都改成直接调用
